...
public class Game1 : Game
{
    ...
    protected override void Initialize()
    {
        try
        {
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"Screen resolution: {viewport.Width}x{viewport.Height}");
                System.Diagnostics.Debug.WriteLine($"Preferred resolution: {_graphics.PreferredBackBufferWidth}x{_graphics.PreferredBackBufferHeight}");
                System.Diagnostics.Debug.WriteLine($"Aspect ratio: {(float)viewport.Width / viewport.Height}");
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine("Game1 Initialize complete");
            }
            catch { /* Ignore platform-specific logging errors */ }
        }
        catch (System.Exception ex)
    ...
    protected override void LoadContent()
    {
        try
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"GLTRON: Failed to load menu XNB: {ex.Message}");
            ...
            try
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine("3D graphics components initialized successfully");
                }
                catch { /* Ignore platform-specific logging errors */ }

                // HUD message for model path removed to keep screen clean.
            }
            catch (System.Exception ex)
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine($"3D graphics initialization failed: {ex}");
                }
                catch { /* Ignore platform-specific logging errors */ }
            ...
            try
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine("Sound initialized and music started");
                }
                catch { /* Ignore platform-specific logging errors */ }
            }
            catch (System.Exception ex)
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine($"Sound init failed: {ex.Message}");
                    System.Diagnostics.Debug.WriteLine("Continuing without sound - game will still work");
                }
                catch { /* Ignore platform-specific logging errors */ }
                // Continue without sound - this is non-critical
            }

            System.Diagnostics.Debug.WriteLine("LoadContent completed successfully");
        }
        catch (System.Exception ex)
        {
            try { System.Diagnostics.Debug.WriteLine($"LoadContent failed: {ex}"); } catch { }
    ...
    {
        // Touch-only controls: no keyboard/gamepad handling
        ...
        {
            try { System.Diagnostics.Debug.WriteLine($"RunGame error: {ex}"); } catch { }
        }
    ...
    protected override void Draw(GameTime gameTime)
    {
        try
        {
            ...
            if (GraphicsDevice == null)
            {
                System.Diagnostics.Debug.WriteLine("GraphicsDevice is null in Draw!");
                return;
            }
            ...
            {
                System.Diagnostics.Debug.WriteLine("SpriteBatch is null in Draw!");
                return;
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"Draw - isInMenu: {isInMenu}, _worldGraphics: {_worldGraphics != null}, _camera: {_camera != null}");
            }
            catch { }
            ...
            if (!isInMenu && _worldGraphics != null && _camera != null)
            {
                try
                {
                    try
                    {
                        System.Diagnostics.Debug.WriteLine("Setting up 3D rendering pipeline");
                    }
                    catch { }
                    ...
                    try
                    {
                        ...
                        try
                        {
                            System.Diagnostics.Debug.WriteLine("Screen cleared with black for 3D game");
                        }
                        catch { }
                    }
                    catch (System.Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"Simple clear failed: {ex.Message}");
                        }
                        catch { }
                        ...
                        try
                        {
                            ...
                            try
                            {
                                System.Diagnostics.Debug.WriteLine("Fallback red clear worked");
                            }
                            catch { }
                        }
                        catch (System.Exception ex2)
                        {
                            try
                            {
                                System.Diagnostics.Debug.WriteLine($"All clear methods failed: {ex2.Message}");
                            }
                            catch { }
                        }
                    }
                    ...
                    if (player != null)
                    {
                        try
                        {
                            ...
                            if (_glTronGame.IsCameraResetRequested() && _camera != null)
                            {
                                ...
                                try
                                {
                                    System.Diagnostics.Debug.WriteLine("Camera reset handled - immediate positioning enabled");
                                }
                                catch { }
                            }
                            ...
                            try
                            {
                                System.Diagnostics.Debug.WriteLine($"Camera following player at: X={x:F2}, Y={y:F2}");
                            }
                            catch { }
                        }
                        catch (System.Exception ex)
                    ...
                    try
                    {
                        ...
                        if (player != null)
                        {
                            try
                            {
                                ...
                                try
                                {
                                    System.Diagnostics.Debug.WriteLine($"Camera updated with turn interpolation - pos: {playerPos}, dir: {playerDirection}, lastDir: {lastDirection}");
                                }
                                catch { }
                            }
                            catch (System.Exception ex)
                    ...
                    catch (System.Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"Camera update failed: {ex.Message}");
                        }
                        catch { }
                    ...
                    try
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"Camera View matrix: {_camera.View}");
                            System.Diagnostics.Debug.WriteLine($"Camera Projection matrix: {_camera.Projection}");
                        }
                        catch { }
                        ...
                        try
                        {
                            System.Diagnostics.Debug.WriteLine("WorldGraphics.BeginDraw called successfully");
                        }
                        catch { }
                    }
                    catch (System.Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"WorldGraphics.BeginDraw FAILED: {ex.Message}");
                        }
                        catch { }
                    ...
                    try
                    {
                        ...
                        try
                        {
                            System.Diagnostics.Debug.WriteLine("Skybox drawn successfully");
                        }
                        catch { }
                    }
                    catch (System.Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"DrawSkybox FAILED: {ex.Message}");
                        }
                        catch { }
                    }
                    ...
                    try
                    {
                        ...
                        try
                        {
                            System.Diagnostics.Debug.WriteLine("Floor drawn successfully");
                        }
                        catch { }
                    }
                    catch (System.Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"DrawFloor FAILED: {ex.Message}");
                        }
                        catch { }
                    }
                    ...
                    if (walls != null)
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                System.Diagnostics.Debug.WriteLine($"Arena grid size: {gridSize}");
                                
                                for (int i = 0; i < walls.Length && i < 4; i++)
                                {
                                    if (walls[i] != null)
                                    {
                                        System.Diagnostics.Debug.WriteLine($"Wall {i}: start=({walls[i].vStart.v[0]:F1},{walls[i].vStart.v[1]:F1}) dir=({walls[i].vDirection.v[0]:F1},{walls[i].vDirection.v[1]:F1})");
                                    }
                                }
                            }
                            catch { }
                            ...
                            try
                            {
                                System.Diagnostics.Debug.WriteLine("Walls drawn successfully");
                            }
                            catch { }
                        }
                        catch (System.Exception ex)
                        {
                            try
                            {
                                System.Diagnostics.Debug.WriteLine($"DrawWalls FAILED: {ex.Message}");
                            }
                            catch { }
                        }
                    }
                    else
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine("Walls array is null!");
                        }
                        catch { }
                    }
                    ...
                    if (_glTronGame?.DrawRecognizer() == true)
                    {
                        ...
                        if (recognizer != null)
                        {
                            try
                            {
                                ...
                                try
                                {
                                    System.Diagnostics.Debug.WriteLine("Recognizer drawn successfully");
                                }
                                catch { }
                            }
                            catch (System.Exception ex)
                            {
                                try
                                {
                                    System.Diagnostics.Debug.WriteLine($"DrawRecognizer FAILED: {ex.Message}");
                                }
                                catch { }
                            }
                        }
                            ...
                            if (players[i] != null)
                            {
                                try
                                {
                                    ...
                                    if (players[i].getSpeed() > 0.0f || players[i].getExplode())
                                    {
                                        ...
                                        try
                                        {
                                            System.Diagnostics.Debug.WriteLine($"Drawing player {i} bike at ({x:F1},{y:F1}) speed={players[i].getSpeed():F1}");
                                        }
                                        catch { }
                                ...
                                catch (System.Exception ex)
                                {
                                    try
                                    {
                                        System.Diagnostics.Debug.WriteLine($"Draw player {i} FAILED: {ex.Message}");
                                    }
                                    catch { }
                                }
                            }
                    ...
                    catch (System.Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"EndDraw FAILED: {ex.Message}");
                        }
                        catch { }
                    }
                }
                ...
                {
                    try { System.Diagnostics.Debug.WriteLine($"3D rendering error: {ex}"); } catch { }
            ...
            else
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine("Menu mode - cleared with black background");
                }
                catch { }
            }
            ...
            try
            {
                ...
                if (_font != null)
                {
                    if (isInMenu)
                    {
                        ...
                        // Touch controls instruction
                        var controlsText = "Swipe left/right to turn";
                        var controlsSize = _font.MeasureString(controlsText);
                        _spriteBatch.DrawString(_font, controlsText,
                            new Vector2(centerX - controlsSize.X / 2, centerY + 130), Color.Cyan);

                        var pinchText = "Pinch to zoom • Swipe up/down to change speed";
                        var pinchSize = _font.MeasureString(pinchText);
                        _spriteBatch.DrawString(_font, pinchText,
                            new Vector2(centerX - pinchSize.X / 2, centerY + 160), Color.LightGray);
                    }
                    else
                    {
                        ...
                        // Touch controls only (keyboard removed)
                        _spriteBatch.DrawString(_font, "Swipe left/right to turn", new Vector2(10, viewport.Height - 80), Color.Yellow);
                        _spriteBatch.DrawString(_font, "Swipe up/down to change speed", new Vector2(10, viewport.Height - 60), Color.Cyan);
                    }
                }
                else
                {
            ...
            {
                try { System.Diagnostics.Debug.WriteLine($"2D UI rendering error: {ex}"); } catch { }
            }

        }
        catch (System.Exception ex)
        {
            try { System.Diagnostics.Debug.WriteLine($"Draw method error: {ex}"); } catch { }
        }

        try
        {
            System.Diagnostics.Debug.WriteLine("About to call base.Draw(gameTime)");
        }
        catch { }
        ...
        try
        {
            System.Diagnostics.Debug.WriteLine("Completed base.Draw(gameTime)");
        }
        catch { }
    }
            ...
            foreach (TouchLocation touch in touchCollection)
            {
                switch (touch.State)
                {
                    case TouchLocationState.Pressed:
                        ...
                        // Recognizer toggle (top-left corner tap)
    ...
    private void ProcessSwipeGesture(Vector2 endPosition, GameTime gameTime)
    {
        try
        {
            ...
            {
                float norm = MathHelper.Clamp(-swipeVector.Y / GraphicsDevice.Viewport.Height, -1f, 1f);
                _currentSpeed = MathHelper.Clamp(_currentSpeed + (norm * 0.5f), MIN_SPEED, MAX_SPEED);
                _glTronGame?.SetAllPlayersSpeed(_currentSpeed);
                return;
            }
            ...
            else
            {
                ...
                System.Diagnostics.Debug.WriteLine("Vertical swipe detected (not used for turning)");
            }
        }
        catch (System.Exception ex)
        {
    ...
    private void ProcessTurnInput(int turnDirection)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"ProcessTurnInput error: {ex}");
        }
    }
}
